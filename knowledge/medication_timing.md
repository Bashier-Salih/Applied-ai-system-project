...
When a pet has more than one medication, some combinations are meant to be spaced apart (for example, certain antibiotics and antacids can interfere with each other's absorption if taken together). If a schedule places two different medication-type tasks back-to-back with no gap, that's worth flagging for the owner to double check with their vet, even though it isn't a "conflict" in the scheduling sense.

## Missed doses
A single missed dose of most non-critical medications is not usually an emergency, but repeated missed doses of medications for conditions like seizures or heart disease should be discussed with a vet promptly.
